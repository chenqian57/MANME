...
def validate(model, args, vocab, output_type="rankings", max_retrieve = 6, split='test'):
	"""
	Input:
		model, args, vocab;
...
def compute_and_process_compatibility_scores(data_loader_query, data_loader_target,
										model, args, output_type="rankings",
										max_retrieve=6):
	"""
	...
	for data in tqdm(data_loader_query):
		...
		for i, index in enumerate(indices):
			...
			cs = cs + cs_14 + cs_28
			#print('cs:', cs.shape)
			#print('GT_indices:', GT_indices.size)
			...
			print('result:', ret[index, :max_retrieve])
	return ret
...
def compute_necessary_embeddings_img(data_loader_target, model, args):
	...
	for data in tqdm(data_loader_target):
		...
		#print("img_emb:", img_trg_emb)
		#print("img_embsize:", img_trg_emb.size)
		...
		if img_trg_embs is None:
			...
			#emb7_sz = [len(data_loader_target.dataset), 7*7, args.embed_dim]

			#img_trg_embs_7 = torch.zeros(emb7_sz, dtype=img_trg_emb.dtype, requires_grad=False)
			if torch.cuda.is_available():
